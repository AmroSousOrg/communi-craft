...
export const ExternalApiComponent = () => {
  ...
  const callApi = async () => {
    try {
      ...
      // const response = await fetch("http://localhost:8080/api/users/"/*`${apiOrigin}/api/external`*/, {
      //   headers: {
      //     Authorization: `Bearer ${token}`,
      //   },
      // });

      const responseData = `Bearer ${token}`;
  ...
  return (
    <>
      ...
      <Button
          color="primary"
          className="mt-5"
          onClick={() => {navigator.clipboard.writeText(state.apiMessage)}}
          disabled={!audience}
        >
          Copy
        </Button>
